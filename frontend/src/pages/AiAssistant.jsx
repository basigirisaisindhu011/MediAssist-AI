...
export const AiAssistant = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto h-[calc(100vh-7.5rem)] flex flex-col space-y-4">
      {/* Header Bar */}
      <div className="glass-panel p-5 bg-gradient-to-r from-sky-600/90 via-teal-600/90 to-indigo-600/90 text-white rounded-2xl shadow-xl flex items-center justify-between shrink-0">
      ...
      {/* Main Chat Container */}
      <div className="glass-card flex-1 p-4 sm:p-6 overflow-y-auto flex flex-col space-y-4 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-lg">
        {/* Messages List */}
        <div className="flex-1 space-y-4">
                  ...
                  <div
                    className={`p-4 rounded-2xl text-sm leading-relaxed ${
                      isUser
                        ...
                        : 'bg-slate-100 dark:bg-slate-800/90 text-slate-900 dark:text-slate-100 rounded-tl-none border border-slate-200/80 dark:border-slate-700/60 shadow-sm'
                    }`}
                  >
                    {msg.text}
                  </div>
          ...
          {loading && (
            <div className="flex items-center space-x-3 animate-fade-in">
              ...
              <div className="p-3.5 rounded-2xl rounded-tl-none bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-xs flex items-center space-x-2 border border-slate-200 dark:border-slate-700">
          ...
          <div className="flex flex-wrap gap-2">
            {suggestedQuestions.map((q, idx) => (
              <button
                ...
                className="px-3 py-1.5 rounded-full text-xs bg-slate-100 hover:bg-sky-50 dark:bg-slate-800 dark:hover:bg-slate-700/80 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 transition-colors text-left"
              >
                {q}
              </button>
            ))}
      ...
      {/* Input Box */}
      <div className="glass-card p-3 flex items-center space-x-2 border border-slate-200 dark:border-slate-800 rounded-2xl shrink-0">
        <input
          type="text"
          value={input}
          ...
          className="flex-1 px-4 py-2.5 bg-transparent text-slate-900 dark:text-slate-100 text-sm focus:outline-none"
        />
        <button
          type="button"
          onClick={() => handleSend()}
          disabled={loading || !input.trim()}
          className="p-3 bg-gradient-to-r from-sky-600 to-teal-600 hover:from-sky-700 hover:to-teal-700 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-xl shadow-md transition-all shrink-0"
        >
      ...
      <div className="p-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-800 dark:text-amber-300 text-[11px] flex items-center space-x-2 shrink-0">
